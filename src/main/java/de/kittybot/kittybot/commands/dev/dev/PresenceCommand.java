...
public class PresenceCommand extends SubCommand{
	...
	public void run(Options options, Interaction ia){
		...
		if(activity == Activity.ActivityType.STREAMING && message == null){
			ia.error("The message may not be empty for streaming");
			return;
		}
		ia.getJDA().getPresence().setPresence(status, activity == null ? null : Activity.of(activity, message == null ? "" : message, streamingURL));
		ia.reply("Successfully set presence");
	}
