...
public class PlayCommand extends ACommand{
	...
	public void run(CommandContext ctx){
		...
		if(voiceState != null && voiceState.inVoiceChannel()){
			var currentPlayer = Cache.getMusicPlayer(ctx.getGuild());
			if (currentPlayer != null){
				currentPlayer.loadItem(this, ctx, ctx.getArgs());
				return;
			}
			...
			MusicPlayer newPlayer = new MusicPlayer(player);
			player.addListener(newPlayer);
			Cache.addMusicPlayer(ctx.getGuild(), newPlayer);
			newPlayer.loadItem(this, ctx, ctx.getArgs());
		}
		else{
