...
public class CuddleCommand extends Command{
	...
	public void run(String[] args, GuildMessageReceivedEvent event){
		...
		if(id != null){
			...
			if(user != null){
				try{
					String url = API.getNeko("cuddle");
					EmbedBuilder eb = new EmbedBuilder();
					eb.setColor(Color.pink);
					if(user.getId().equals(event.getAuthor().getId())){
						eb.setDescription(user.getAsMention() + " cuddles **himself**!");
					}
					else{
						eb.setDescription("**" + API.getNameByUser(event.getMember()) + "** cuddles " + user.getAsMention() + "!");
					}
					eb.setImage(url);
